...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // Store original reservation values
    const originalDate = dateInput.value;
    const originalTimeSlot = timeSlotInput.value;
    const originalGuests = guestInput.value;
    ...
    // If date is set, load time slots
    if (dateInput.value) {
        loadTimeSlots(dateInput.value, originalTimeSlot);
    }
    ...
    let currentGuests = parseInt(originalGuests);
    updateGuestCount();
     ...
     * @param {string} selectedSlotId - The ID of the currently selected time slot
     */
    async function loadTimeSlots(date, selectedSlotId = null) {
        try {
            ...
            if (response.ok) {
                renderTimeSlots(data.available_slots, selectedSlotId);
            } else {
     ...
     * @param {string} selectedSlotId - The ID of the currently selected time slot
     */
    function renderTimeSlots(slots, selectedSlotId = null) {
    ...
    form.addEventListener('submit', function(e) {
        ...
        const hasChanges = (
            ...
            timeSlotInput.value !== originalTimeSlot ||
            guestInput.value !== originalGuests ||
            form.name.value !== "{{ reservation.name }}" ||
            form.email.value !== "{{ reservation.email }}" ||
            form.phone.value !== "{{ reservation.phone }}" ||
            form.special_requests.value !== "{{ reservation.special_requests|default:'' }}"
        );
